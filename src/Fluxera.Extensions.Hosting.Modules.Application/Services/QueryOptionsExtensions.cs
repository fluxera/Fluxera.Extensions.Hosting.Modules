namespace Fluxera.Extensions.Hosting.Modules.Application.Services
{
	...
	internal static class QueryOptionsExtensions
	{
			...
			where TDto : class
		{
			...
			{
				Expression<Func<TAggregateRoot, object>> mappedOrderByExpression = mapper.MapExpression<Expression<Func<TAggregateRoot, object>>>(sortingOptions.PrimaryExpression.Expression);
				...
				if(sortingOptions.SecondaryExpressions.Any())
				{
					...
					{
						Expression<Func<TAggregateRoot, object>> mappedThenByExpression = mapper.MapExpression<Expression<Func<TAggregateRoot, object>>>(secondaryExpression.Expression);
			...
			where TDto : class
		{
			...
			if(options.TryGetPagingOptions(out IPagingOptions<TDto> pagingOptions))
			{
				...
				Repository.Query.IPagingOptions<TAggregateRoot> mappedPagingOptions = Repository.Query.QueryOptions<TAggregateRoot>.Paging(pageNumberAmount, pageSizeAmount);
